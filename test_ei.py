...
def main():
    ...
    for url in existing_urls:
        ...
        entities_df = pd.read_pickle('data/tmp/en2_df.pkl')
        entities_df = entities_df[entities_df['entity_category'].isin(config.en_to_keep)]
        entities_df['entity'] = entities_df['entity'].apply(lambda x: remove_char(config.special_chars, x))
        ...
        for idx in idx_lst:
            ...
            sub['en_obj'] = sub['object'].astype(str).apply(lambda x: [en for en in en_lst if en in x])
    ...
    try:
        ...
        # Insert all vertices
        print("Let's insert some vertices into the graph")
        insert_vertices(client, _gremlin_insert_vertices)

        # Create edges between vertices
        print("Insert edges into the graph")
